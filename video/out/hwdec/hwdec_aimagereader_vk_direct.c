...
// libplacebo may request nearest sampling even for a 1:1 plane read.
// Both sample modes must work with the same immutable chroma conversion.
static const VkFormatFeatureFlags required_sampling_features =
...
{
    if (!aimagereader_vk_format_depth(props->format))
        return false;
    ...
    *features = format_props.formatProperties.optimalTilingFeatures;
    return (*features & required_sampling_features) == required_sampling_features &&
    ...
    int32_t data_space)
{
    ...
    depth = MPMAX(depth, aimagereader_vk_format_depth(props->format));
    ...
    struct source_config *source)
{
    *source = (struct source_config) {
    ...
    if ((selected_features & required_sampling_features) !=
        required_sampling_features) {
        mp_verbose(p->log, "Android hardware buffer cannot be sampled "
                           "directly with independent sample filtering\n");
        return AIMAGEREADER_VK_MAP_UNSUPPORTED;
    }

    aimagereader_vk_apply_data_space(&source->format_props, data_space);
    source->raw_dovi =
