                              ...
                              int *release_fence, bool *release_exported)
{
    ...
    *release_exported = false;
    ...
    if (acquire_imported) {
        ...
        // Foreign ownership acquisition is not scoped to the shader stage.
        wait_stages[num_waits++] = VK_PIPELINE_STAGE_ALL_COMMANDS_BIT;
    }
    if (wait_for_available) {
        wait_semaphores[num_waits] = output->available;
        // Also order the output layout transition after libplacebo's use.
        wait_stages[num_waits++] = VK_PIPELINE_STAGE_ALL_COMMANDS_BIT;
    }
